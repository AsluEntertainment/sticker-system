...
app.post('/api/pedidos', async (req, res) => {
  const { cliente, numero_pedido, fecha_entrega, items } = req.body;
  const finalId = numero_pedido || Date.now().toString();
...
// Completa el pedido guardando los valores finales
app.post('/api/pedidos/:id/completar', async (req, res) => {
  const { id } = req.params;
  const { total_stickers, numero_entrega, zona_produccion, slot_almacen } = req.body;

  try {
    const updateData = {
      estado: 'completado',
      fin_produccion: new Date()
    };

    if (total_stickers !== undefined) updateData.total_stickers = total_stickers;
    if (numero_entrega !== undefined) updateData.numero_entrega = numero_entrega;
    if (zona_produccion !== undefined) updateData.zona_produccion = zona_produccion;
    if (slot_almacen !== undefined) updateData.slot_almacen = slot_almacen;
...
app.get('/api/stats', async (req, res) => {
  ...
  try {
    ...
    let query = supabase
      .from('pedidos')
      .select(`*, pedido_colecciones(*)`)
      .order('creado_en', { ascending: true }); // Fallback sorting

    // We can filter minimally at DB level if necessary, 
    // but the actual dates to group are paqueteria or produccion.
    if (fechaDesde) {
      query = query.gte('fecha_produccion', fechaDesde);
    }

    const { data: pedidos, error: errorPedidos } = await query;
    if (errorPedidos) throw errorPedidos;

    // ── Nombres y Qty de colecciones ──────────────────────────────────────────
    const { data: colecciones } = await supabase.from('colecciones').select('id, nombre, cantidad_stickers');
    const colMap = {};
    (colecciones || []).forEach(c => { 
      colMap[c.id] = { nombre: c.nombre, cantidad_stickers: c.cantidad_stickers || 0 }; 
    });
    ...
    for (const p of listaPedidos) {
      const cols = p.pedido_colecciones || [];
      const stickersPedido = (p.total_stickers && Number(p.total_stickers)) || 0;
      ...
      if (p.inicio_produccion && p.fin_produccion) {
        const tiempoHrs = (new Date(p.fin_produccion) - new Date(p.inicio_produccion)) / (1000 * 60 * 60);
        tiemposProduccion.push({
          pedido_id:        p.id,
          tiempo_horas:     Math.round(tiempoHrs * 100) / 100,
          cantidad_stickers: stickersPedido,
          fecha:            p.fecha_paqueteria || p.fecha_produccion || p.creado_en 
        });
      }
      ...
      for (const c of cols) {
        ...
        const factor = (colMap[c.coleccion_id] && colMap[c.coleccion_id].cantidad_stickers) ? parseInt(colMap[c.coleccion_id].cantidad_stickers, 10) : 0;
        const unidades = parseInt(c.cantidad, 10) || 0;
        conteoColecciones[c.coleccion_id] += (unidades * factor);
      }
    }
    ...
    const porColeccion = Object.entries(conteoColecciones)
      .map(([coleccion_id, cantidad]) => ({
        coleccion_id,
        nombre:      (colMap[coleccion_id] && colMap[coleccion_id].nombre) ? colMap[coleccion_id].nombre : coleccion_id,
        cantidad
      }))
    ...
    for (let i = 7; i >= 0; i--) {
      ...
      const pedidosSem = listaPedidos.filter(p => {
        const d = new Date(p.fecha_paqueteria || p.fecha_produccion || p.creado_en);
        return d >= inicioSem && d < finSem;
      });

      const stickersSem = pedidosSem.reduce((s, p) => s + (p.total_stickers || 0), 0);
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const stickersMes = pedidosMes.reduce((s, p) => s + (p.total_stickers || 0), 0);
