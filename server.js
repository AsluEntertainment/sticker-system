...
app.get('/api/estadisticas', async (req, res) => {
  ...
  try {
    ...
    for (const p of listaPedidos) {
      ...
      const stickersPedido = cols.reduce((s, c) => s + (c.cantidad || 0), 0);
      ...
      if (p.inicio_produccion && p.fin_produccion) {
        ...
        tiemposProduccion.push({
          ...
          fecha:            p.creado_en ? p.creado_en.split('T')[0] : ''
        });
      }
      ...
      for (const c of cols) {
        ...
        conteoColecciones[c.coleccion_id] += c.cantidad || 0;
      }
    }
    ...
    const porColeccion = Object.entries(conteoColecciones)
      .map(([coleccion_id, cantidad]) => ({
        coleccion_id,
        nombre:      colMap[coleccion_id] || coleccion_id,
        cantidad
      }))
    ...
    for (let i = 7; i >= 0; i--) {
      ...
      const stickersSem = pedidosSem.reduce((s, p) =>
        s + (p.pedido_colecciones || []).reduce((ss, c) => ss + (c.cantidad || 0), 0), 0
      );
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const stickersMes = pedidosMes.reduce((s, p) =>
        s + (p.pedido_colecciones || []).reduce((ss, c) => ss + (c.cantidad || 0), 0), 0
      );
