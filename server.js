...
app.post('/api/pedidos', async (req, res) => {
  ...
  try {
    ...
    const { data: nuevo, error: errorPedido } = await supabase
      ...
      .insert({
        id: finalId,
        nombre_receptor: cliente || 'Sin Nombre',
...
// Completa el pedido con total_stickers y fecha_paqueteria
app.post('/api/pedidos/:id/completar', async (req, res) => {
  const { id } = req.params;
  const { total_stickers, fecha_paqueteria, slot_almacen } = req.body;
  ...
  try {
    const updateData = {
      ...
      fecha_paqueteria: fecha_paqueteria || new Date().toISOString()
    };
    
    if (slot_almacen && slot_almacen !== '---') {
      updateData.slot_almacen = slot_almacen;
    }
